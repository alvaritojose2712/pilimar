...
export default function PedidosCentralComponent({
	...
	return (
		<div className="container">
			<button className="btn btn-outline-sinapsis" onClick={()=>{setInventarioFromSucursal();setsubviewcentral("")}}>Exportar inventario a Central</button>
			<button className="btn btn-outline-sinapsis" onClick={()=>{getInventarioFromSucursal();setsubviewcentral("inventario")}}>Importar inventario a Central</button>
			<button className="btn btn-outline-sinapsis" onClick={()=>{getPedidosCentral();setsubviewcentral("pedidos")}}>Importar Pedidos</button>
				...
				<table className="table">
					...
					<tbody>
						{inventarioModifiedCentralImport.length ? inventarioModifiedCentralImport.map((e, i) =>
							<tr key={i} className={(e.check ? "bg-success-light" : "bg-danger-light") + (" pointer")} >
								<td className="cell05">
